                ...
                <form action="{{ route('projects.workers.store', $project) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="position" 
                            ...
                            <option value="현장소장" {{ old('position') == '현장소장' ? 'selected' : '' }}>현장소장</option>
                            <option value="안전관리자" {{ old('position') == '안전관리자' ? 'selected' : '' }}>안전관리자</option>
                            <option value="근로자" {{ old('position') == '근로자' ? 'selected' : '' }}>근로자</option>
                        </select>
                        @error('position')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
